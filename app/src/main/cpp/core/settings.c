...
#if !defined EMU28_INI
	#define EMU28_INI "Emu28.ini"
#endif

#define ReadString(sec,key,dv,v,sv) GetPrivateProfileString(sec,key,dv,v,sv,_T(EMU28_INI))
#define ReadInt(sec,key,dv)         GetPrivateProfileInt(sec,key,dv,_T(EMU28_INI));
#define WriteString(sec,key,v)      WritePrivateProfileString(sec,key,v,_T(EMU28_INI))
#define WriteInt(sec,key,v)         WritePrivateProfileInt(sec,key,v,_T(EMU28_INI))
#define DelKey(sec,key)             WritePrivateProfileString(sec,key,NULL,_T(EMU28_INI))
...
// registry handling
	...
	#define REGISTRYKEY "Software\\Emu28"
#endif
...
{
	// Files
	ReadString(_T("Files"),_T("Emu28Directory"),szCurrentDirectory,szEmuDirectory,ARRAYSIZEOF(szEmuDirectory));
	...
	dwSaturnCycles      = ReadInt(_T("Emulator"),_T("SaturnCycles"),dwSaturnCycles);
...
{
	// Files
	WriteString(_T("Files"),_T("Emu28Directory"),szEmuDirectory);
	...
	WriteInt(_T("Emulator"),_T("SaturnCycles"),dwSaturnCycles);
