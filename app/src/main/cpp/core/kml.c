...
static VOID CreateKmlList(VOID)
{
	...
	do
	{
		...
		bInvalidPlatform = (szTitle == NULL || lstrcmpi(_T(HARDWARE),szTitle) != 0);
...
static KmlLine* InitAnnunciator(KmlBlock* pBlock)
{
	...
	while (pLine)
	{
		...
		case TOK_SIZE:
			...
			break;
		case TOK_OPAQUE:
			pAnnunciator[nId].bOpaque = (BOOL) pLine->nParam[0];
			break;
		case TOK_END:
			return pLine;
		default:
...
VOID DrawAnnunciator(UINT nId, BOOL bOn, DWORD dwColor)
{
	...
	EnterCriticalSection(&csGDILock);		// solving NT GDI problems
	{
		if (bOn)
		{
			HDC hDC = hAnnunDC != NULL ? hAnnunDC : hMainDC;

			// draw annunciator with transparent background
			if (pAnnunciator[nId].bOpaque == FALSE)
			{
				HDC      hBmpDC;
				HBITMAP  hBmp;
				COLORREF cColor;
				...
				// annunciator symbol must be black
				cColor = SetBkColor(hDC,GetPixel(hDC,pAnnunciator[nId].nDx,pAnnunciator[nId].nDy));
				BitBlt(hBmpDC,
					0, 0,
...
static VOID ResizeMainBitmap(INT nMul, INT nDiv)
{
	if (nMul * nDiv > 0)					// resize main picture
	{
		...
		for (i = 0; i < ARRAYSIZEOF(pAnnunciator); ++i)
		{
			...
			{
				// translate down
				pAnnunciator[i].nDx = (UINT) MulDiv(pAnnunciator[i].nDx,nMul,nDiv)-xOff;
				pAnnunciator[i].nDy = (UINT) MulDiv(pAnnunciator[i].nDy,nMul,nDiv)-yOff;
				// translate size
		...
		EnterCriticalSection(&csGDILock);	// solving NT GDI problems
		{
			...
			hMainBitMap = CreateCompatibleBitmap(hMainDC,nWidth,nHeight);
			hMemBitMap = (HBITMAP) SelectObject(hMemDC,SelectObject(hMainDC,hMainBitMap));

			// create a main bitmap with HALFTONE stretch mode
			...
			if (hAnnunDC == NULL)			// no external annunciator bitmap
			{
				...
				// width and height of annunciator source bitmap
				UINT w = rcAnunnciator.right  - rcAnunnciator.left;
				UINT h = rcAnunnciator.bottom - rcAnunnciator.top;

				// width and height of annunciator destination bitmap
				UINT nAnnunWidth  = (UINT) MulDiv(w,nMul,nDiv);
				UINT nAnnunHeight = (UINT) MulDiv(h,nMul,nDiv);

				// create an annunciator bitmap with BLACKONWHITE stretch mode
				VERIFY(hAnnunDC = CreateCompatibleDC(hWindowDC));
				hAnnBitmap = (HBITMAP) CreateCompatibleBitmap(hMemDC,nAnnunWidth,nAnnunHeight);
				hAnnBitmap = (HBITMAP) SelectObject(hAnnunDC,hAnnBitmap);

				VERIFY(StretchBlt(
					hAnnunDC,0,0,nAnnunWidth,nAnnunHeight,
					hMemDC,rcAnunnciator.left,rcAnunnciator.top,w,h,
					SRCCOPY));

				SetAnnunBitmap(hAnnunDC,hAnnBitmap);
			}
...
quit:
	if (bOk)
	{
		// HP28C has object loading -> support dropped files
		DragAcceptFiles(hWnd,cCurrentRomType == 'P');
