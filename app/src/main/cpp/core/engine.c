...
DWORD  dwSaturnCycles = 39;					// Saturn cpu cycles in interval
...
static __inline VOID Debugger(VOID)			// debugger part
{
	...
	if (bDbgCode && (Chipset.pc == 0x02CA8 || Chipset.pc == 0x02D06))
	{
		enum MMUMAP eMap;
		...
		if (eMap != M_ROM)					// address not in ROM
	...
	// RPL breakpoint, PC=(A), opcode #808C
	if (I[0] == 0x8 && I[1] == 0x0 && I[2] == 0x8 && I[3] == 0xC) // 1LK7
	{
		// get next RPL entry
		DWORD dwAddr = Npack(Chipset.A,5);	// 1LK7, there's no PC=(C) opcode
	...
	if (bStopEmulation)						// stop condition
	{
		...
		StopTimers(MASTER);
		if (Chipset.IORamM[TIMERCTL]&RUN)	// check if master timer running
		{
			if (dwEDbgT2[MASTER] == Chipset.tM)
			{
				// cpu cycles for one master timer tick elapsed
			...
			if (Chipset.IORamS[TIMERCTL]&RUN)
			{
				if (dwEDbgT2[SLAVE] == Chipset.tS)
				{
					// cpu cycles for one slave timer tick elapsed
			...
			dwEDbgT2[SLAVE] = Chipset.tS;	// slave timer check reference value
		}
		dwEDbgT2[MASTER] = Chipset.tM;		// master timer check reference value
