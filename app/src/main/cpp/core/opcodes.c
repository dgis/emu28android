...
VOID o0F(LPBYTE I) // RTI
{
	...
	if (w.intd && (w.intk || w.IR15X))		// 1LK7, keyboard interrupt pending
	{
	...
	if (w.IORamM[TIMERCTL]&INTR)			// INT bit of timer is set
		ReadT(MASTER);						// check for int

	if (w.IORamS[TIMERCTL]&INTR)			// INT bit of timer is set
		ReadT(SLAVE);						// check for int
	return;
}
...
VOID o158x(LPBYTE I) { w.cycles+=15+I[3]+1; w.pc+=4; Nwrite(w.A, w.d0, I[3]+1); return; } // DAT0=A x 1LK7
VOID o159x(LPBYTE I) { w.cycles+=15+I[3]+1; w.pc+=4; Nwrite(w.A, w.d1, I[3]+1); return; } // DAT1=A x 1LK7
VOID o15Cx(LPBYTE I) { w.cycles+=15+I[3]+1; w.pc+=4; Nwrite(w.C, w.d0, I[3]+1); return; } // DAT0=C x 1LK7
VOID o15Dx(LPBYTE I) { w.cycles+=15+I[3]+1; w.pc+=4; Nwrite(w.C, w.d1, I[3]+1); return; } // DAT1=C x 1LK7
VOID o15Ax(LPBYTE I) { w.cycles+=16+I[3]+1; w.pc+=4; Nread(w.A, w.d0, I[3]+1); return; } // A=DAT0 x 1LK7
VOID o15Bx(LPBYTE I) { w.cycles+=16+I[3]+1; w.pc+=4; Nread(w.A, w.d1, I[3]+1); return; } // A=DAT1 x 1LK7
VOID o15Ex(LPBYTE I) { w.cycles+=16+I[3]+1; w.pc+=4; Nread(w.C, w.d0, I[3]+1); return; } // C=DAT0 x 1LK7
VOID o15Fx(LPBYTE I) { w.cycles+=16+I[3]+1; w.pc+=4; Nread(w.C, w.d1, I[3]+1); return; } // C=DAT1 x 1LK7
...
VOID o801(LPBYTE I) // OUT=C
{
	...
	if (((wOut ^ w.out) & 0xC00) != 0)		// beeper bits OR[11:10] changed
		SoundOut(&w,wOut);
	w.out = wOut;
	ScanKeyboard(FALSE,FALSE);				// 1ms keyboard poll
	return;
...
VOID o807(LPBYTE I) // SHUTDN
{
	...
	if (w.IORamM[TIMERCTL]&WKE)				// WKE bit of timer is set
	{
		if (ReadT(MASTER)&0x80000000)		// and MSB of timer is set
		{
			w.IORamM[TIMERCTL] &= ~WKE;		// clear WKE
			bShutdn = FALSE;				// don't shut down
		}
	}

	if (w.IORamS[TIMERCTL]&WKE)				// WKE bit of timer is set
	{
		if (ReadT(SLAVE)&0x80000000)		// and MSB of timer is set
		{
			w.IORamS[TIMERCTL] &= ~WKE;		// clear WKE
			bShutdn = FALSE;				// don't shut down
		}
	}
...
VOID o8080(LPBYTE I) // INTON
{
	...
	w.intk = TRUE;
	ScanKeyboard(FALSE,FALSE);				// 1ms keyboard poll
	if (w.intd)								// 1LK7, keyboard interrupt pending
	{
		w.intd = FALSE;						// no keyboard interrupt pending any more
		INTERRUPT;							// restart interrupt handler
	}
...
VOID o80E(LPBYTE I) // SREQ?
{
	...
	w.C[0] = ((w.IORamM[TIMERCTL] | w.IORamS[TIMERCTL]) & SRQ) != 0;
	if (w.C[0]) w.HST |= SR;
	return;
}
...
VOID oBb0(LPBYTE I) { w.cycles+=3+F_l[I[1]&7]; w.pc+=3; NFsl(w.A, I[1]&7); return; } // 1LK7
VOID oBb1(LPBYTE I) { w.cycles+=3+F_l[I[1]&7]; w.pc+=3; NFsl(w.B, I[1]&7); return; } // 1LK7
VOID oBb2(LPBYTE I) { w.cycles+=3+F_l[I[1]&7]; w.pc+=3; NFsl(w.C, I[1]&7); return; } // 1LK7
VOID oBb3(LPBYTE I) { w.cycles+=3+F_l[I[1]&7]; w.pc+=3; NFsl(w.D, I[1]&7); return; } // 1LK7
...
VOID oBb4(LPBYTE I) { w.cycles+=3+F_l[I[1]&7]; w.pc+=3; NFsr(w.A, I[1]&7); return; } // 1LK7
VOID oBb5(LPBYTE I) { w.cycles+=3+F_l[I[1]&7]; w.pc+=3; NFsr(w.B, I[1]&7); return; } // 1LK7
VOID oBb6(LPBYTE I) { w.cycles+=3+F_l[I[1]&7]; w.pc+=3; NFsr(w.C, I[1]&7); return; } // 1LK7
VOID oBb7(LPBYTE I) { w.cycles+=3+F_l[I[1]&7]; w.pc+=3; NFsr(w.D, I[1]&7); return; } // 1LK7
