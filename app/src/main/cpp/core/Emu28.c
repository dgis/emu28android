...
#else
LPCTSTR szNoTitle = _T("Emu28 ")_T(VERSION);
#endif
LPTSTR szAppName = _T("Emu28");				// application name for DDE server
...
{
	// enable stack loading on HP28C
	BOOL bStackEnable = (cCurrentRomType == 'P');
...
static LRESULT OnDropFiles(HDROP hFilesInfo)
{
	...
	if (!(Chipset.IORamM[DSPCTL]&DON))		// calculator off, turn on
	{
		// turn on HP
...
static LRESULT OnViewScript(VOID)
{
	...
	if (nState != SM_RUN)
	{
		InfoMessage(_T("You cannot change the KML script when Emu28 is not running.\n")
...
{
	ShellExecute(hWnd,_T("open"),_T("Emu28.htm"),NULL,szEmuDirectory,SW_SHOWNORMAL);
	return 0;
}
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpCmdLine, int nCmdShow)
{
	...
	wc.cbWndExtra = 0;
	wc.hInstance = hInst;
	wc.hIcon = LoadIcon(hInst, MAKEINTRESOURCE(IDI_EMU28));
	wc.hCursor = NULL;
	wc.hbrBackground = NULL;
	wc.lpszMenuName = MAKEINTRESOURCE(IDR_MENU);
	wc.lpszClassName = _T("CEmu28");

	if (!(classAtom = RegisterClass(&wc)))
	{
		AbortMessage(
			_T("CEmu28 class registration failed.\n")
			_T("This application will now terminate."));
		return FALSE;
	}
	...
	hWnd = CreateWindow(MAKEINTATOM(classAtom),_T("Emu28"),
		STYLE_TITLE,
