...
static CHIPSET OldChipset;					// old chipset content of master (CPU)
...
static VOID SetMappingMenu(HWND hDlg,UINT uID)
{
	...
	case ID_DEBUG_MEM_MAP:
		...
		break;
	case ID_DEBUG_MEM_ROM:
		szCaption = _T("Memory (ROM)");
		eMode = MEM_ROM;
		break;
	case ID_DEBUG_MEM_RAM_M:
		szCaption = _T("Memory (RAM master)");
		eMode = MEM_RAMM;
		break;
	case ID_DEBUG_MEM_IO_M:
		szCaption = _T("Memory (I/O master)");
		eMode = MEM_IOM;
		break;
	case ID_DEBUG_MEM_RAM_S:
		szCaption = _T("Memory (RAM slave)");
		eMode = MEM_RAMS;
		break;
	case ID_DEBUG_MEM_IO_S:
		szCaption = _T("Memory (I/O slave)");
		eMode = MEM_IOS;
		break;
	default: _ASSERT(0);
	}
...
static VOID UpdateMmuWnd(HWND hDlg)
{
	...
	if (Chipset.RamCfigM)
		wsprintf(szBuffer,_T("%05X"),Chipset.RamBaseM << 10);
	else
		lstrcpy(szBuffer,_T("-----"));
	SetDlgItemText(hDlg,IDC_MMU_RAM_M,szBuffer);
	if (Chipset.IOCfigM)
		wsprintf(szBuffer,_T("%05X"),Chipset.IOBaseM << 10);
	else
		lstrcpy(szBuffer,_T("-----"));
	SetDlgItemText(hDlg,IDC_MMU_IO_M,szBuffer);
	if (Chipset.RamCfigS)
		wsprintf(szBuffer,_T("%05X"),Chipset.RamBaseS << 10);
	else
		lstrcpy(szBuffer,_T("-----"));
	SetDlgItemText(hDlg,IDC_MMU_RAM_S,szBuffer);
	if (Chipset.IOCfigS)
		wsprintf(szBuffer,_T("%05X"),Chipset.IOBaseS << 10);
	else
		lstrcpy(szBuffer,_T("-----"));
	SetDlgItemText(hDlg,IDC_MMU_IO_S,szBuffer);
	return;
}
...
static INT_PTR CALLBACK Debugger(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		SendMessage(hDlg,WM_SETICON,ICON_BIG,(LPARAM) LoadIcon(hApp,MAKEINTRESOURCE(IDI_EMU28)));
		...
		SetMappingMenu(hDlg,uIDMap);		// init memory mapping
	...
	case WM_COMMAND:
		...
		case ID_DEBUG_MEM_IO_M:
		case ID_DEBUG_MEM_RAM_S:
		case ID_DEBUG_MEM_IO_S:           return OnMemMapping(hDlg,LOWORD(wParam));
...
static VOID UpdateProfileWnd(HWND hDlg)
{
	...
	lCpuFreq = T2CYCLES * 16384;
	lEngFreq = (999 * 2 * lCpuFreq - lCpuFreq) / 2 / 1000;

	// search for ENG unit
	for (i = 0; i < ARRAYSIZEOF(pcUnit) - 1 && lVar <= lEngFreq; ++i)
	{
		lVar *= 1000;						// next ENG unit
	}

	// calculate rounded time
	lVar = (2 * lVar + lCpuFreq) / (2 * lCpuFreq);
...
static INT_PTR CALLBACK RplObjView(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_INITDIALOG:
		dwRplPlatform = RPL_P1;				// Clamshell platform without RRP
...
static VOID OutTrace(VOID)
{
	...
	if (hLogFile != NULL)					// log file opened
	{
		...
		if (bTraceMmu)						// show MMU
		{
			...
			wsprintf(szAddr, Chipset.RamCfigM ? _T("%05X") : _T("-----"),Chipset.RamBaseM << 10);
			PrintTrace(_T("  RAM_M=%s"),szAddr);

			wsprintf(szAddr, Chipset.IOCfigM ? _T("%05X") : _T("-----"),Chipset.IOBaseM << 10);
			PrintTrace(_T("  I/O_M=%s"),szAddr);

			wsprintf(szAddr, Chipset.RamCfigS ? _T("%05X") : _T("-----"),Chipset.RamBaseS << 10);
			PrintTrace(_T("  RAM_S=%s"),szAddr);

			wsprintf(szAddr, Chipset.IOCfigS ? _T("%05X") : _T("-----"),Chipset.IOBaseS << 10);
			PrintTrace(_T("  I/O_S=%s\r\n"),szAddr);
		}
