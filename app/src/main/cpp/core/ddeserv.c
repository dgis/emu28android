							  ...
							  DWORD dwData1,DWORD dwData2)
{
	...
	switch (iType)
	{
		...
		case XTYP_POKE:
			// all none HP28C or illegal data format or not in running state
			if (cCurrentRomType != 'P' || iFmt != uCF_HpObj || nState != SM_RUN)
			...
			if (!(Chipset.IORamM[DSPCTL]&DON)) // HP off
			{
				// turn on HP
...
cancel:
		...
		case XTYP_REQUEST:
			...
			// if the calculator is off the object is one stack level higher,
			// because stack level 1 contain the FALSE object in this case
			if ((Chipset.IORamM[DSPCTL]&DON) == 0)
				++nStkLvl;
			...
			lpHeader = (LPBYTE) BINARYHEADER28C;
